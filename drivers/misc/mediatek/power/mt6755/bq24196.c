...
static void __maybe_unused bq24196_shutdown(struct platform_device *dev)
{
...
static DEFINE_MUTEX(bq24196_access_mutex);
...
int bq24196_read_byte(kal_uint8 cmd, kal_uint8 *returnData)
{
	...
	char readData = 0;
	int ret = 0;
//	__u16 addr =  new_client->addr ;
//	__u16 addr_temp = 0;
//	const char temp[16] = {0};
...
/*
	do {
		ret = i2c_master_send(new_client, &temp[0], (1 << 8 | 1));
		if (ret >= 0) {
			        printk("[bq24196] addr=0x%x success. \n", addr_temp);
		}
			addr_temp++;
			new_client->addr = addr_temp;
	} while (addr_temp < 0x7f);
				    ...
				    kal_uint8 SHIFT)
{
	...
	pr_debug("--------------------------------------------------\n");
        mutex_lock(&bq24196_access_mutex);
	ret = bq24196_read_byte(RegNum, &bq24196_reg);
	...
	ret = bq24196_write_byte(RegNum, bq24196_reg);
         mutex_unlock(&bq24196_access_mutex);
...
void bq24196_dump_register(void)
{
	...
	for (i = 0; i < bq24196_REG_NUM; i++) {
		...
		printk( "[bq24196] [0x%x]=0x%x ", i, bq24196_reg[i]);
	}
	printk( "[bq24196] dump end.\n");
}

unsigned int bq24196_hw_component_detect(void)
{
        unsigned int ret = 0;
        unsigned char val = 0;
...
extern kal_int32 bq24196_chr_control_interface(CHARGING_CTRL_CMD cmd, void *data);
static int bq24196_driver_probe(struct i2c_client *client, const struct i2c_device_id *id) {
	unsigned int ret = 0;
	...
	ret = bq24196_hw_component_detect();
	if (ret == 0)
		return -EIO;
	...
	battery_charging_control = bq24196_chr_control_interface;
	return 0;
}

static int __maybe_unused bq24196_driver_probe_old(struct i2c_client *client, const struct i2c_device_id *id)
{
	int err = 0;
...
static struct i2c_board_info __maybe_unused i2c_bq24196 __initdata = {
...
static int __init bq24196_init(void)
{
	int ret = 0;
#ifdef CONFIG_OF
        printk( "[bq24196_init] init start with i2c DTS \n");
#else

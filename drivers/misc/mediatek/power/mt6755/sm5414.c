...
static struct i2c_client new_client_body;
...
static DEFINE_MUTEX(sm5414_access_mutex); 
/**********************************************************
  *
...
unsigned int sm5414_config_interface (unsigned char RegNum, unsigned char val, unsigned char MASK, unsigned char SHIFT)
{
    ...
    mutex_lock(&sm5414_access_mutex);
    ...
    mutex_unlock(&sm5414_access_mutex);
    // Check
...
void __weak __maybe_unused charger_en_gpio_output( int level){}
...
#endif
extern int sm5414_chr_control_interface(CHARGING_CTRL_CMD cmd, void *data);
...
{             
    ...
    new_client = &new_client_body;
    new_client_body.addr = SM5414_CHG_SLAVE_ADDR_WRITE / 2;

    sm5414_cnt_nshdn_pin(1); //Set nSHDN pin to high for i2c

    ret = sm5414_hw_component_detect();
    if (ret == 0) {
        battery_xlog_printk(BAT_LOG_CRTI,"[sm5414_driver_probe] pmic not found\n"); 
	new_client = NULL;
	return -EIO;
    }
    //---------------------
    ...
    battery_charging_control = sm5414_chr_control_interface;
...
static int of_i2c_dev_notify(struct notifier_block *nb,
...
static struct notifier_block i2c_dev_of_notifier;
static int __init __maybe_unused overlay_insmod(void)
{
        i2c_dev_of_notifier.notifier_call = of_i2c_dev_notify;
        return /*retval = */of_reconfig_notifier_register(&i2c_dev_of_notifier);
}

//late_initcall(overlay_insmod);
